...
@interface ItemsLiked ()

@end

@implementation ItemsLiked
...
- (void)viewDidLoad
{
    ...
    strImageShareUrl = @"";
    lblItemsLiked.font = FONT_Lato_Bold(17.0f);
}

#pragma mark - viewWillAppear Method
- (void) viewWillAppear:(BOOL)animated {
    KAppDelegate.MyVC = self;
    [self GetItemsLikedData];
}

// here we get items liked data................
- (void) GetItemsLikedData {
    NSString * UserId = @"";
    ...
    if([obNet InternetStatus:YES]) {
        NSString *url = [NSString stringWithFormat:@"%@&UserId=%@",[KAppDelegate getDictServer:WS_StuffLikes],UserId];
        NSLog(@"stuffLikes Url-->%@",url);
        [obNet JSONFromWebServices:url Parameter:nil AI:YES PopUP:NO WithBlock:^(id json){
            NSLog(@"stuffLikes Json value-->%@",json);
...
- (void) add_AdjustViewProductListing {
    ...
    for(int i = 0; i< arrItemListing.count; i++){
        ...
        NSString * str = @"";
        if([obNet isObject:[dict objectForKey:@"ProductAddDate"] String_1_Array_2_Dictionary_3_Integer_4_UIImage_5:1]) {
            str = [dict objectForKey:@"ProductAddDate"];
        }
        ...
        @try {
            lblProducrPrice.text = [NSString stringWithFormat:@"$ %d",[[dict objectForKey:@"ProductPrice"] intValue]];
        } @catch (NSException *exception) { }
        
        if([obNet isObject:[dict objectForKey:@"ProductLikeCount"] String_1_Array_2_Dictionary_3_Integer_4_UIImage_5:4]) {
...
// button to share image on social...............
